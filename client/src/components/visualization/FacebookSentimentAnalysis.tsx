...
interface FacebookSentimentAnalysisProps {
  platformData: PlatformData;
  isLoading?: boolean;
}

export function FacebookSentimentAnalysis({ platformData, isLoading }: FacebookSentimentAnalysisProps) {
  if (isLoading) {
    return (
      <Card className="mb-8">
    ...
    <Card className="bg-white rounded-lg overflow-hidden mb-8">
      <CardContent className="p-6">
        <h3 className="text-lg font-semibold mb-4 flex items-center">
          <SiFacebook className="mr-2 text-[#1877F2]" />
          Facebook Sentiment Analysis
        </h3>
        
        <p className="text-gray-600 mb-6">
          This analysis examines the emotional tone of your Facebook content:
        </p>
