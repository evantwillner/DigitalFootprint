...
export class CacheService<T> {
  ...
  private cacheLevels: {[key:string]:number}

  constructor(options: { maxSize?: number; defaultTTL?: number; cacheLevels?: {[key:string]:number} } = {}) {
    this.cache = new Map();
    this.maxSize = options.maxSize || 1000; // Default max 1000 items
    this.defaultTTL = options.defaultTTL || 3600000; // Default 1 hour
    this.cacheLevels = options.cacheLevels || {default: this.defaultTTL};
  ...
  /**
   * Set a value in the cache with a custom TTL and cache level
   */
  set(key: string, value: T, ttlOrLevel: string | number = 'default', cacheLevel?:string ): void {
    let ttl: number;
    if(typeof ttlOrLevel === 'string'){
        ttl = this.cacheLevels[ttlOrLevel] || this.defaultTTL;
    } else {
        ttl = ttlOrLevel;
    }
   ...
   */
  get(key: string, cacheLevel:string = 'default'): T | null {
...
export const platformDataCache = new CacheService<PlatformData | null>({
  maxSize: 5000,
  cacheLevels: {
    PROFILE: 60 * 60 * 1000, // 1 hour for profile data
    ACTIVITY: 15 * 60 * 1000, // 15 minutes for activity data
